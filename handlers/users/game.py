...
async def show_field(message: Message):
    global table_dict, cntr

    table_dict = {i: k for i, k in zip(range(1, 10), "123456789")}
    cntr = 0
    await message.answer(text="Начнем игру! Выберите ваш знак",
...
async def choice_sign(call: CallbackQuery, callback_data: dict):
    ...
    token = player
    await call.message.answer(text="Выберите число",
...
async def nums_choice(call: CallbackQuery, callback_data: dict):
    global table_dict, cntr, token

    cntr += 1
    text = "Ходит игрок"
    data = callback_data["item_name"]

    answer = plc_sign(token, data, table_dict)
    ...
    if cntr > 3:
        if check_win(table_dict):
            await call.message.edit_text(
                f"{check_win(table_dict)} - Выиграл{chr(127942)}{chr(127881)}!",
                reply_markup=fun_choice(table_dict))
            await restart(call)

    if cntr == 9:
        await call.message.edit_text(f"Ничья! {chr(129318)}{chr(129309)}!",
                                     reply_markup=fun_choice(table_dict))
        await restart(call)
